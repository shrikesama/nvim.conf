...
local function detect_cwd(buf)
  ...
  local cwd
  if sys:find('linux') then cwd = Detect.linux(pid)
  elseif sys:find('darwin') then cwd = Detect.darwin(pid)
  elseif sys:find('windows') or F.has('win32') == 1 then cwd = Detect.windows(pid)
  end
